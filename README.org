...
1. Clone this repository:
   ...
   cd my_project
   #+END_SRC
2. Configure the project and build the dependencies
   #+BEGIN_SRC bash
   cd my_project_directory
   cmake -B build .
   #+END_SRC 
3. Compile the project and load it onto the microcontroller
   #+BEGIN_SRC bash
...
- Startup code is stored in ~startup/~
  - The interrupt vector table (see [[*Startup Library][Startup Library]]) likely requires adjustment and is stored in <model>_ivt.c
- Linker scripts for ~gcc~ are stored in ~startup/<model>-GCC.lds~  and ~startup/<model>-TI.cmd~ 
