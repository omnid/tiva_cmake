...
- Use ~find_package(TivaCMake COMPONENTS None)~ to check for the existence of ~tiva-cmake~ without actually configuring anything.
  This feature is useful when the host system (non-cross-compiling) wants to know if ~tiva-cmake~ exists without loading any
