...
:END:
The startup library (~TivaCMake::startup~) contains code that runs before ~main()~ to initialize the microcontroller,
...
1. The startup code is a CMake OBJECT library, that must be directly linked with your executable to ensures that all
   the code in the library is included after the final linking.  
2. If you use ~TivaCMake::startup~ as a dependency of a library, you must specify it as having ~INTERFACE~ linkage, not
   ~PUBLIC~ or ~PRIVATE~.  If you use ~PUBLIC~ or ~PRIVATE~, the code is linked into the new library you are making and
   then it won't be linked in fully to the executable.
