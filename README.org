...
It enables the use of ~arm-none-eabi-gcc~ or TI's Code Generation Tools (~ti-cgt~) and also provides an easy way to write your
code to flash memory using the in-circuit debugger (ICD) on TI's LaunchPad development boards. 
It also comes bundled with ~TivaWare~ ~driverlib~.

Use ~tiva-cmake~ as a [[*Template Mode (No Installation)][template]] or [[*Installation][install]] it (either to your home directory or system-wide) and use it as a dependency. 

When using ~tiva-cmake~ as a template you must manually install the [[*Development Tools][Development Tools]]. 
Some installation methods automatically install the required tools. 
You do not need [[https://www.ti.com/tool/CCSTUDIO][Code Composer Studio]] to use ~tiva-cmake~; however, installing it is one method to obtain all of the necessary tools. 

* Template Mode (No Installation)
Make sure you have the necessary [[*Development Tools][Development Tools]] installed. 
The example assumes that you have an ~EK-TM4C123GXL~ plugged into a USB port.
...
The project resides in ~src/~, which is the sole location you should need to edit.
...
- See [[*Startup Code][Startup Code]] for information about how to implement interrupt service routines.
