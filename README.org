...
3. A buildtool (such as make or ninja)
4. A cross compiler.  [[https://www.ti.com/tool/CCSTUDIO][Code Composer Studio]] provides everything needed to build and flash your program.
   - However, Code Composer Studio is a large program and it may be desirable to obtain your tools elsewhere (see below).
*** Windows Users
Here is the recommended installation:
1. Install [[https://www.msys2.org][MSYS2]]
2. MSYS2 uses the ~pacman~ package manager from Arch Linux, so you can proceed to follow the Arch Linux installation instructions from
   within MSYS2
** GNU GCC Toolchain
