...
2. Copy the toolchain file you need to the example directory (e.g., =toolchain/TivaCMake/GNU-toolchain.cmake=)
...
*** Toolchain Details
1. CMake is designed to require passing the full path to a toolchiain file
2. The =tiva_cmake= package comes with two files: =toolchain/TivaCMake/TI-toolchain.cmake= and
   =toolchain/TivaCMake/GNU-toolchain.cmake= that are designed to be distributed with your project.
   - These files, when passed as the =toolchain= will find the location of the =tiva_cmake= toolchain
     that is installed on the system (e.g., on the =${CMAKE_INSTALL_PREFIX}= path) and find the appropriate toolchain.
   - They can also be copied directly from this repository and distributed with your code.
3. There are also scripts: =arm-none-eabi-gcc-toolchain= and =ti-cgt-arm-toolchain= that are installed into the =bin/= directory
   that output the full path to the toolchain files.
...
8. =DRIVERLIB_BUILD_TYPE=: Defaults to =Release=. Setting to =Debug= enables debugging TivaWare driverlib.
