...
:END:
** Dependencies
1. [[https://cmake.org/download][CMake]]  (Version 3.28 or later).
2. There are a few options for obtaining the cross compiler. The toolchain supports =GNU= and =TI= compilers:
   1. GCC on Ubuntu: =sudo apt install  gcc-arm-none-eabi libnewlib-arm-none-eabi gdb-multiarch=
   2. GCC on ArchLinux: =sudo pacman -S arm-none-eabi-gcc arm-none-eabi-newlib arm-none-eabi-gdb=
   3. GCC on Windows/Mac: [[https://developer.arm.com/Tools%20and%20Software/GNU%20Toolchain][Arm GNU Toolchain]]
   4. TI Code Generation Tools: [[https://www.ti.com/tool/ARM-CGT]]
      - Install to =/opt= or =/home/$(whoami)=
      - On ArchLinux can be installed from the AUR as =ti-cgt-arm=.
   5. Alternatively, [[https://www.ti.com/tool/CCSTUDIO][Code Composer Studio]] provides everything that is needed.
      - This is a large download but might be easiest on Windows/Mac.
4. Flash Tools:
   1. [[https://openocd.org][OpenOCD]] on Ubuntu: =sudo apt install openocd= and ArchLinux: =sudo pacman -S openocd=
   2. [[https://www.ti.com/tool/UNIFLASH]]
   3. Uniflash is also distributed with Code Composer Studio.
